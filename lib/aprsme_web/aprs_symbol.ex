defmodule AprsmeWeb.AprsSymbol do
  ...
  def get_overlay_base_symbol_info(base_symbol_code) do
    ...
    base_symbol_ord = base_symbol_code
                      |> String.to_charlist()
                      |> List.first()
                      |> (fn c -> if is_integer(c), do: c, else: 63 end).()
  ...
  def get_overlay_base_table_id(base_symbol_code) do
    ...
    case base_symbol_code do
      ...
      "#" -> "1"  # Digipeater - green star background
      "a" -> "1"  # Diamond shape - APRS overlay symbol (alternate table)
      "A" -> "1"  # Square shape - APRS overlay symbol (alternate table)
      "&" -> "1"  # Diamond shape - alternate table
      ">" -> "1"  # Arrow symbols
      "<" -> "1"
      "^" -> "1"
      "v" -> "1"
      "i" -> "1"  # Black square background - alternate table
  ...
  def get_table_id(symbol_table) do
    case symbol_table do
      "/" -> "0"    # Primary table
      "\\" -> "1"   # Alternate table
      "]" -> "2"    # Overlay table (A-Z, 0-9)
      _ -> "0"      # Default to primary table
    end
  end
